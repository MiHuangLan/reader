...
def parse_args():
    ...
    train_settings.add_argument('--l2_norm', type=float, default=3e-7,
                                help='l2 norm')
    train_settings.add_argument('--clip_weight', type=bool, default=False,
                                help='clip weight')
    train_settings.add_argument('--max_norm_grad', type=float, default=5.0,
                                help='max norm grad')
    train_settings.add_argument('--dropout', type=float, default=0.7,
                                help='dropout rate')
    ...
    model_settings.add_argument('--algo_match', choices=['BIDAF', 'MLSTM'], default='MLSTM',
    ...
    path_settings.add_argument('--train_files', nargs='+',
                               default=['./data/corpus/mneydata/search.train.json'],
                               help='list of files that contain the preprocessed train data')
    path_settings.add_argument('--dev_files', nargs='+',
                               default=['./data/corpus/mneydata/search.dev.json'],
                               help='list of files that contain the preprocessed dev data')
    path_settings.add_argument('--test_files', nargs='+',
                               default=['./data/corpus/mneydata/search.test1.json'],
                               help='list of files that contain the preprocessed test data')
    path_settings.add_argument('--save_dir', default='./data/corpus/mneydata/baidu',
                               help='the dir with preprocessed baidu reading comprehension data')
    path_settings.add_argument('--vocab_dir', default='./data/corpus/mneydata/vocab/',
                               help='the dir to save vocabulary')
    path_settings.add_argument('--model_dir', default='./data/corpus/mneydata/models/',
                               help='the dir to store models')
    path_settings.add_argument('--result_dir', default='./data/corpus/mneydata/results/',
                               help='the dir to output the results')
    path_settings.add_argument('--summary_dir', default='./data/corpus/mneydata/summary/',
                               help='the dir to write tensorboard summary')
    path_settings.add_argument('--log_path',
                               help='path of the log file. If not set, logs are printed to console')
    path_settings.add_argument('--pretrained_word_path',default='sgns.financial.bigram-char',
                               help='path of the log file. If not set, logs are printed to console')
...
def train(args):
    ...
    with io.open(os.path.join(args.vocab_dir, 'vocab.data'), 'rb') as fin:
    ...
    model = MYModel(vocab, args)

    logger.info('Training the model...')

    model.train(dataloader, args.epochs, args.batch_size, save_dir=args.model_dir, save_prefix=args.algo, dropout_keep_prob=args.dropout)
...
def predict(args):
    ...
    test_batches = dataloader.gen_mini_batches('test', args.batch_size, vocab.get_word_id(vocab.pad_token), shuffle=False)
    # print('###')
