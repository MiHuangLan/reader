...
from match_layer import MatchLSTMLayer
...
class MYModel(object):
    ...
    def _setup_placeholders(self):
        ...
        self.c_mask = tf.cast(self.p, tf.bool)  # index 0 is padding symbol  N x self.max_p_num, max_p_len
    ...
    def _match(self):
        ...
        if self.algo_match == 'MLSTM':
            ...
            raise NotImplementedError('The algorithm {} is not implemented.'.format(self.algo_match))
    ...
    def _train_epoch(self, train_batches, dropout_keep_prob):
        ...
        for bitx, batch in enumerate(train_batches, 1):
            ...
            #print('1111')
            #print(batch)
            #ara_tokens = word_tokenize(batch['passage_token_ids'])
            # question_tokens = word_tokenize(batch['question_token_ids'])
            # try:
            #     recall_wrt_question = sentence_bleu(para_tokens, question_tokens[0], weights=(0.25, 0.25, 0.25, 0.25))
            #     sum=0
            #     for s in batch['passage_length']:
            #         sum=sum+s
            #     num=recall_wrt_question/sum
            #     #print(num)
            #
            # except KeyError:
            #     pass
